...
let modSidesPromise = null;

/**
 * Per-mod side map for multi-mod essences (Hysteria primarily).
 * Generated by scripts/update-poe2-essences.sh from poe2db's per-class
 * Pre/Suf table. Shape:
 *   { mod_sides: { "Essence of Hysteria": { "#% increased ...": "PREFIX", ... } } }
 * The keys are canonicalised mod text (#-form), matching extra_mods.json.
 */
export function loadEssenceModSides() {
  if (!modSidesPromise) {
    modSidesPromise = fetch(new URL('../../data/poe2/essence_mod_sides.json', import.meta.url))
      .then((r) => {
        if (!r.ok) throw new Error(`Failed to load essence_mod_sides.json (${r.status})`);
        return r.json();
      })
      .catch(() => ({ mod_sides: {} })); // Tolerate missing file (older data dump).
  }
  return modSidesPromise;
}
